...
# Teardown
CustomUser.objects.filter(username='test_student_log_v2').delete()

try:
    # 1. Create User & Student
    user = CustomUser.objects.create_user(username='test_student_log_v2', password='password')
    student = StudentProfile.objects.create(user=user, student_name='Test Student Log V2', student_number=9998)
    ...
    if logs.count() != 2:
        ...
        for log in logs:
            print(f"  Log: {log.status} at {log.time}")
